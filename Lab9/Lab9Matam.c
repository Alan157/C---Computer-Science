...
int treeHight(TreeNode* tree)// returns the hight of a given tree
{
	...
	return Max(treeHight(tree->left) + 1, treeHight(tree->right) + 1);
}
...
TreeNode* create_tree_node(Data data)
{
	...
	if ((new_tree) == NULL) {
		printf("No memo");
		return NULL; //Return null, insert_info_by_key will free all the memory of the tree
	}
	new_tree->TreeNode_info.key = data.key;
	new_tree->TreeNode_info.val = data.val;
	new_tree->left = NULL;
	new_tree->right = NULL;
}
